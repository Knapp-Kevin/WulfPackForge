...
- `editorModeBar.py` owns the vanilla-friendly appearance mode: its large on/off switch and banner, `apply(window)` which pushes the mode into the tabs (read-only inventory and skills, the palette picker on the Appearance tab, the Advanced HDR / Overbright group hidden with overbright switched off, the naming rule on the Misc tab), and `blocking_reason(window)`, the one sentence that refuses a save outside the mode's rules. `skinPaletteDialog.py` is the palette picker over `data/skinPalette.py` (the ten Monk Skin Tone hues at the game's lightness, the nudge, and the membership test), `subscripts/editorMode.py` remembers the mode in `<workspace>/settings.json`, and `saveGuards.py` holds the Valheim-running guard the main window calls before a save. The mode is a policy layer over the tabs: no write path reads the flag, and the saved file carries no trace of it.
