...
### Added
- Valheim 1.0 save files are now read. The version 46 container that 1.0 introduced is parsed and written byte-identically, including the ten statistics records it added. Editing a 1.0 character is not yet possible — its player data uses a newer layout — but the file is no longer rejected with a byte count.
