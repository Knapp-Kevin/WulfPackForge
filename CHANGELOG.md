...
- The beard sits on the face for every hairstyle (see the preview entry under Added); a contact-sheet tool (`tools/preview_contact_sheet.py`) renders every hair with each beard so the placement can be checked by eye.
...
- Beards and moustaches in the head preview are fitted to the face: the beard art is scaled and shifted so its ear line lands on the hairstyle's ear line, instead of being centred or fitted to the shoulders, both of which had left beards sitting high or drifting with the hairstyle.
