...
### Added
- A **Vanilla-friendly appearance mode**, a large on/off switch under the action row for players of vanilla servers such as Jotunheim that allow appearance edits only. With it on, skin comes from a palette of ten realistic tones (the Monk Skin Tone scale, Google, 2023, CC BY 4.0, at the game's own lightness) with a small lightness nudge, hair and beard colours stay at or below 1.1, the Advanced HDR / Overbright controls are hidden, the Inventory and Skills tabs are read-only, and a renamed character needs each word capitalised; a save outside those rules is refused with one sentence. New characters made in the mode start from a palette tone and follow the naming rule. The mode is a courtesy, not enforcement: it can be switched off, and the file carries no trace of it. It is remembered per computer in the workspace's `settings.json`.
- A release validation runbook (`docs/release-validation-runbook.md`) for the steps only a person with the game can perform, and a save comparison tool (`tools/compare_saves.py`) that names every field the game changed in a file Wulfpack Forge wrote, so in-game acceptance is recorded evidence rather than an impression. The README gains a supported-platforms statement: Windows is validated and packaged; macOS and Linux run from source and are unverified as player platforms.
- The item catalog is now generated from the player's own Valheim installation and refreshed to **Valheim 1.0.12**: 1517 items including every Deep North and gold-weapon addition, each carrying the game's stack, quality and variant limits, durability maximum and per-level increment, and damage table. The JotunnDoc item list is kept as a cross-check. Items that were shown as `unknown#…` in a 1.0 save, such as the Frostfire Spear and Sword, are now named.
- The item editor warns when an item's damage at the chosen quality is above 10000, the bound Valheim itself uses to mark an item as cheated.
- Valheim 1.0 characters open and save. The 1.0 player data (version 33) and its rewritten inventory record (version 109) are read and written byte-identically: items are named through the game's own stable hash, an item the catalog cannot name keeps a stable `unknown#<hash>` identity and is written back under the same hash, and the build-interface state 1.0 appends to the save passes through untouched. Every 1.0 character on the developer's machine round-trips byte-for-byte; in CI the layouts are validated against a fixture derived from those saves.
- An **Achievement risk** section on the Record tab. It shows what the character file records about cheat state — the profile flag, how many items carry the game's cheated mark, and whether the 1.0.12 achievements opt-in has been used — and states plainly that world cheat state and modded play are not in the file. Wulfpack Forge writes none of these: existing marks pass through unchanged and an item the editor adds is written clean.
- Valheim 1.0 save files are read. The version 46 container that 1.0 introduced is parsed and written byte-identically, including the ten statistics records it added.
- Crash diagnostics. If Wulfpack Forge is ever killed by a fault below Python — the kind that closes the window with no message and no error in the log — the Python stack of every running thread is now written to a crash report next to the log file, and the next launch tells you it happened and where the report is, with a button that opens the folder. Reports are kept private to your account, the newest five are retained, and a normal exit leaves none behind. Qt's own warnings, errors raised while Python is cleaning up, and failures inside background workers are now written to the log as well, instead of being discarded. If the report file cannot be created the log says so plainly rather than leaving you to assume you are covered.
- macOS and Linux readiness: a `run-wulfpack-forge.sh` source launcher that mirrors the Windows one; Steam, the game folder (including the macOS `valheim.app` layout for Game Icons), and mod-manager profiles (r2modman under Application Support on macOS, under the config folder on Linux) are looked up where each platform keeps them; the test suite now runs on macOS in CI as well as Linux. Windows remains the validated platform; macOS has not yet been tested with a real game install.
...
### Changed
- The version marker is 0.9.0-rc.2. The earlier rc.1 draft was built before any Valheim 1.0 work and is superseded, not published.
- Durability maxima now come from the game data in the catalog rather than a table scraped from the community wiki, which disagreed with the game on 53 of its 213 entries (Mistlands and Ashlands weapons most of all). The percent-of-maximum durability editor therefore shows and writes correct values for those items.
- Known biomes on the Record tab are named for 1.0 characters too. Valheim 1.0 stores them as text, sometimes as a token (`$biome_meadows`) and sometimes as a plain name, and one character can hold both spellings of the same biome; each biome is now listed once by its name, and anything unrecognised is shown as stored.
- A save this build cannot read now says why. A character whose player data is newer than this build understands reports the version it carries, instead of the internal message about how many bytes were expected. A file that is genuinely damaged still reports the original detail, because that is the truthful answer for corruption.
- The character list and the editor now agree. Previously a file could be listed with its real name and then fail to open; both paths now reach the same verdict.
- The R, G and B letters in the Advanced HDR / Overbright Colors rows now sit beside their value boxes instead of inside them, so a row reads "Skin RGB: R 2.000 G 1.500 B 0.250" and each box shows only its number. Screen readers still announce the channel, because each letter is bound to its box and each box is named for its row and channel.

### Fixed
- A character is one entry again. Valheim 1.0 rewrites a character's creation stamp on every load, which had split one character into two records and two workspace folders; records and workspaces are now keyed on the player id alone, the creation date stays on the Record tab, and the duplicate workspace folders are merged into one at the next launch without deleting anything.
- The picker now says which file is the live save and where it lives: the newest verified active file across Valheim's own folder and Steam's userdata folder is labelled "Active save" with its folder named, and "copy" is used only for Wulfpack Forge's own snapshots and backups. With Steam Cloud on, Valheim 1.0 keeps the live file in the Steam folder, so the old "Steam Cloud (local copy)" label was wrong about exactly the file that mattered.
- A save this build can read is no longer reported as unreadable because an older build refused it: the discovery cache now records which save versions the build supports and re-verifies files when that changes, and forgets files that no longer exist.
- The "previous session crashed" notice appears once per crash instead of at every launch; a report is marked as shown after the notice names it, and still counts toward the five reports kept.
- The message after Save Changes is one sentence naming the file and the backup; both full paths are written to the log instead.
- The beard sits on the face for every hairstyle (see the preview entry below); a contact-sheet tool (`tools/preview_contact_sheet.py`) renders every hair with each beard so the placement can be checked by eye.
- The beard placement that shipped with the fix above lifted beards onto the eyes for many hairstyles, because the "widest row of the head" it anchored on is the ears on a bald beard thumbnail but the forehead on a haired one. Beards are now placed by the bust's shoulder line, which both kinds of thumbnail share, with a bound for long hair that hides the shoulders; the placement was chosen by eye on rendered sheets. The tinted patch from the beard thumbnails' crown highlight no longer appears on the hair.
- Fixed the crash that closed Wulfpack Forge with no message while adding an item to the inventory. Rebuilding the inventory grid took each slot out of its layout in a way that made Qt destroy the layout entry while the editor still held a handle to it. Nothing failed at that moment; the damage surfaced later, usually while the item picker was filling its grid, which is why the window vanished during an unrelated action. Measured over repeated runs, the crash went from five runs in six to none.
- The item editor is now released the moment you close it. It was held in a reference cycle, so Python could not free it by reference counting and left it to the garbage collector, which runs at an unpredictable moment - potentially in the middle of another Qt operation. That is the shape of a crash that closes the window with no message, and adding an item to the inventory was the one path that opened it. Every dialog in the editor is now destroyed at a known point, on every exit path, including when you cancel.
- Adding or removing an inventory item no longer assumes the character payload already has an inventory list. It never failed in practice, because every save the parser reads has one, but the add and remove paths disagreed with the load path about it.
- A test shipped with the crash-diagnostics work redirected its temporary workspace with environment variables, which steer the location on Windows and Linux but not on macOS. On a Mac it ran against the real Wulfpack Forge workspace and would fail there. It now redirects the same way the rest of the suite does.
...
- Beards and moustaches in the head preview are fitted to the face: the beard art is scaled by the bust's shoulder width and shifted so its shoulder line lands on the hairstyle's shoulder line, instead of being centred, which had left beards sitting high.
